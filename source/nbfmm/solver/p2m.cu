...
__global__ p2m_weighting(int num_particle,float2* gpuptr_position_,float* gpuptr_weight_,float2* p2m_buffer)
{
...
__global__ p2m_averaging(int base_size_,float2* gpuptr_grid_position_,float* gpuptr_grid_weight_)
{
  ...
  int thread1Dp = thread2Dpy * base_size_ + thread2Dpx;
   ...
   gpuptr_grid_position_[thread1Dp].x=gpuptr_grid_position_[thread1Dp].x/gpuptr_grid_weight_[thread1Dp];
   gpuptr_grid_position_[thread1Dp].y=gpuptr_grid_position_[thread1Dp].y/gpuptr_grid_weight_[thread1Dp];
}
...
void Solver::p2m(int num_particle) {
  ...
  thrust::device_ptr<int2> thrust_working(p2m_workingspace);
   thrust::device_ptr<float2> thrust_gridPos(gpuptr_grid_position_);
   thrust::device_ptr<float2> thrust_gridWei(gpuptr_grid_weight_);
EqualInt2 eqa;
  PlusInt2 plu;

  thrust::reduce_by_key(thrust_index, thrust_index + num_particle, thrust_weighted, thrust_working, thrust_gridPos, eqa, plu);
  thrust::reduce_by_key(thrust_index, thrust_index + num_particle, thrust_weight, thrust_working, thrust_gridWei, eqa, plu);
p2m_averaging<<<kNumBlock_gridwise,kNumThread_gridwise>>>(base_size_,gpuptr_grid_position_,gpuptr_grid_weight_);
