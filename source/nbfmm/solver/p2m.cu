...
void Solver::p2m( const int num_particle ) {
   ...
   //thrust::pair<int2*,float2*> dummy1;
   //thrust::pair<int2*,float*> dummy2;
   EqualInt2 eqa;
   PlusInt2 pluf2;
   thrust::plus<float> pluf;
   thrust::reduce_by_key(thrust_index, thrust_index + num_particle, thrust_weighted, thrust_working, thrust_cellPos, eqa, pluf2);
   thrust::reduce_by_key(thrust_index, thrust_index + num_particle, thrust_weight, thrust_working, thrust_cellWei, eqa,pluf);
