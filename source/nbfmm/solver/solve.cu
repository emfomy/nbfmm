...
void Solver::solve(
  ...
  // Permute input vectors
  permuteInputVector<<<kNumBlock, kNumThread>>>(num_particle, gpuptr_perm_,
  ...
  permuteOutputVector<<<kNumBlock, kNumThread>>>(num_particle, gpuptr_perm_, gpuptr_effect_origin, gpuptr_effect_);
